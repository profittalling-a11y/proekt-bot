"""Pionex API client wrapper (Binance-compatible engine)."""
import time
import hmac
import hashlib
import logging
...
class PionexClient(ExchangeClient):
    """Wrapper for Pionex API (Binance-compatible).

    Pionex uses Binance's matching engine. API endpoints follow
    Binance futures API patterns with Pionex-specific base URL.
    """
    ...
    def __init__(self, api_key: str, api_secret: str, testnet: bool = True):
        ...
        self._setup_session()
        logger.info(f"Pionex client initialized (testnet={testnet})")
    ...
    def _send_request(self, method: str, endpoint: str, params: Dict = None, body: Dict = None) -> Dict:
        ...
        if data.get("code") and data["code"] != 200:
            raise Exception(f"Pionex API Error: {data.get('msg', data)}")
    ...
    def get_klines(self, symbol: str, interval: str, limit: int = 200) -> List[Dict]:
        ...
        pionex_interval = interval_map.get(interval, interval)
        data = self._send_request("GET", "/fapi/v1/klines", {"symbol": symbol, "interval": pionex_interval, "limit": str(limit)})
        return [
